...
const Footer = () => {
  ...
  return (
    <div className="relative flex justify-center gap-4 items-center p-2">
      <div>
        <p className="font-bold md:text-xl text-sm text-[#261FB3] dark:text-white">
          harshkhavale@2025
        </p>
      </div>
      <div className="bg-[#261FB3] dark:bg-white h-8 p-1"></div>
        ...
        <ul className="list-none flex my-4 gap-2">
          {contacts.map((link) => (
            <a
              ...
              key={link.id}
              target="_blank"
              className={` shadow-2xl rounded-full p-1 text-[#261FB3] dark:text-white gap-2 font-medium cursor-pointer `}
            >
              {link.icon}
            </a>
          ))}
